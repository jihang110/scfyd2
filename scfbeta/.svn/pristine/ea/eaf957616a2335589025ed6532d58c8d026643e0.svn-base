$(function() {
	...
	if (!username) {
		window.location.href = "/scfbeta/login.html";
	}
...
function InitLeftMenu() {
	...
	$('#leftMenuList').on('nodeSelected', function(event, data) {
		...
		if(!CloudUtils.isEmpty(data.menuPath)){
			...
			window.location.href = "/scfbeta/" + data.menuPath;
...
function goHome() {
	...
	window.location.href = "/scfbeta/index.html"
}

function logOut() {
	var options = {
		url:"/scfbeta/login/logout",
		data:"{}",
 ...
 */
//function logoShow(){
//	$("#logo").attr("src", "/scfbeta/images/logo.png");
