$(document).ready(function() {
	...
	$('#dueDate').datetimepicker({
        ...
        $("#dueDate").datetimepicker("setStartDate",$("#loanDate").val());
    });
...
window.operateCreditEvents = {
    ...
    'click .remove': function (e, value, row, index) {
    	...
    	case 'orgn': mUrl = '../../orgnLoanCount/delete';mData = '{"recUid":"'+ row.recUid +'"}';break;
...
function creditAjaxRelaShares(){
	var options = {
		...
		callBackFun : function(data) {
			if(data.result==0){
				$("#shareHolderInfoId").html('');
				$.each(data.dataList, function (index, units) {
					$("#shareHolderInfoId").append("<option value="+units.shareHolderId+">" + units.shareName + "</option>");  
				});
			}else{
				bootbox.alert(data.resultNote);
			}
