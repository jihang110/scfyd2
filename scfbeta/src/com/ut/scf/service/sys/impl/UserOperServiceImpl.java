...
//		int hasmobilephone = userDao.hasUserByMobilephone(mobilephone);
		...
		if(hasUser<1){
			...
			int userRoleResult = sysUserRoleMapper.insert(sysUserRole);
	...
	public BaseRespBean updateUser(UserUpdateReqBean userUpdataBean) {
		...
		if(mobilephone != null)
		{
			hasmobilephone = sysuserMapper.countByExample(sysUserExample);
		}
		...
		com.ut.scf.pojo.auto.SysUserRoleExample.Criteria criteriaUserRole = sysUserRoleExample.createCriteria();
		criteriaUserRole.andRoleIdEqualTo(userUpdataBean.getRoleId());
		criteriaUserRole.andUserIdEqualTo(userUpdataBean.getUserId());
		int  resultRole = sysUserRoleMapper.updateByExampleSelective(sysUserRole,sysUserRoleExample);
