...
function ajxa(text) {
	var options = {
		...
		callBackFun : function(data) {
			...
			if (text) {
				let node = $('#menu_tree').treeview('search', text, {exactMatch: false, revealResults:false});
...
function addMenu() {
	$('#myModal').modal('hide');
	let data = CloudUtils.convertStringJson('addMenuForm');
	data = eval("(" + data + ")");
	data.parentId = parentId;
	let menu = $('#parentId').treeview('getSelected', 0);
	if (menu.length == 0) {
...
function modMenu() {
	...
	let menu = $('#menu_tree').treeview('getSelected', 0);
	var options = {
		url : "../../menu/mod",
...
function upMenu() {
	...
	if (menu.length == 0) {
		...
		let temp = $('#menu_tree').treeview('getSiblings', menu[0].nodeId);
		let i = temp.findIndex((a) => a.nodeId > menu[0].nodeId);
		if (i == 0) {// 第一个，不用移动
