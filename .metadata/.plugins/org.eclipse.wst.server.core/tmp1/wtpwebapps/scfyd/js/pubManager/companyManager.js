...
/*	 $.ajax({
 ...
 function saveFun(type){
	 ...
	 //$('#detailForm').data('bootstrapValidator').validate();
	 if(!$('#detailForm,#shareInfoForm').data('bootstrapValidator').isValid()){  
		    //没有通过校验 
		 return false;
...
function ajaxFileUpload(obj){
	...
	if ($("#file").val().length > 0) {
		...
		if(fileType != ".xls"&&fileType != ".xlsx"&&fileType!=".xlsm"&&fileType!=".docx"&&fileType!=".pdf"&&fileType!=".png"&&fileType!=".jpg"){
			bootbox.alert("支持docx、excel、pdf、png、jpg文件,请选择正确的文件类型");
			return false;
