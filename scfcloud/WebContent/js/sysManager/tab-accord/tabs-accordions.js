...
function ajaxNotFlowCount(){
	var data = '{"workItemId":"","roleId":"'+store.get("roleId")+'"}';
	var options = {
...
function ajaxOverFlowCount(){
	var data = '{"workItemId":""}';
	var options = {
			url : 'workflow/getOverItemCount',
			data : data,
			callBackFun : function(data) {
				if(data.result==0){
					$("#overFlowCount").text(data.workItemsCount);
				}else{
					return false;
				}
			}
...
function toNotFlow(){
	window.location.href="bpm/flowManager/projectList.html";
} 

function toOverFlow(){
	window.location.href="bpm/flowManager/handleOverFlowManager.html";
} 
