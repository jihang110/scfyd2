				...
				callBackFun : function(data) {
					if(data.result==0){
						...
						if (flowType ==="F"){
							...
							//加载表格的数据
							FlowMngCommon.initDetTable(data.custNo,"projectTuningModal");
						}else if(flowType ==="L"){
							//立项申请中的信息
							CloudUtils.setForm(data,"proSetupForm");
							//加载表格的数据
							FlowMngCommon.initDetTable(data.custNo,"projectSetupModal");
						}
					}else{
...
//    	            cancelClass : 'btn-small',
    ...
    FlowMngCommon.initDetTable = function (corpId,ModalId){
    	initSupplierBaseTable(corpId,ModalId);
    	initSplFinInfoTable(corpId,ModalId);
    	initCreditLtgInfoTable(corpId,ModalId);
    	initBusiInfoTable(corpId,ModalId);
    	initAttachmentDetails(corpId,ModalId);
    	//立项审批中的担保方信息和立项报告
    	initproSetupDetails(corpId,ModalId);
    };
    
  //初始化供应商信息的表------------------------------------------------------------------------
    function initSupplierBaseTable(corpId,ModalId){
    	initTableEnterprise(corpId,ModalId);//初始化关联企业信息表  
    	initTableShareHolder(corpId,ModalId);//初始化股东信息表
    	initTableCorpAccount(corpId,ModalId);//初始化账户信息表 
    	
    	initLoanApprovalTable(corpId,ModalId)//放款审批中的表格
    }

    function initTableEnterprise(corpId,ModalId) { 
    	$('#'+ModalId+' #tableEnterprise').bootstrapTable('destroy');  
    	$('#'+ModalId+' #tableEnterprise').bootstrapTable({  
             method: "post", 
    ...
    function initTableShareHolder(corpId,ModalId) {
    	$('#'+ModalId+' #tableShareHolder').bootstrapTable('destroy');  
    	$('#'+ModalId+' #tableShareHolder').bootstrapTable({  
             method: "post", 
    ...
    function initTableCorpAccount(corpId,ModalId) { 
    	$('#'+ModalId+' #tableCorpAccount').bootstrapTable('destroy');  
    	$('#'+ModalId+' #tableCorpAccount').bootstrapTable({  
             method: "post", 
    ...
    //初始化供应商财务信息表
    function initSplFinInfoTable(corpId,ModalId) {
    	negativeInitTable(corpId,ModalId);//资产负债表
    	profitInitTable(corpId,ModalId);//利润表
    	cashflowInitTable(corpId,ModalId);//现金流量表
    	bankRecInitTable(corpId,ModalId);//银行对账单
    	taxReturnsInitTable(corpId,ModalId);//纳税申请表
    }

    function negativeInitTable(corpId,ModalId) {
    	$('#'+ModalId+' #negativeList').bootstrapTable('destroy');
    	$('#'+ModalId+' #negativeList').bootstrapTable({
    		method: "post",
            url: "../../negative/list",
            striped: true,  //表格显示条纹 
            pagination: true, //启动分页 
    ...
    function profitInitTable(corpId,ModalId) {
    	$('#'+ModalId+' #profitList').bootstrapTable('destroy');
    	$('#'+ModalId+' #profitList').bootstrapTable({
    		method: "post",
            url: "../../profit/list",
            striped: true,  //表格显示条纹 
            pagination: true, //启动分页 
...
//    	    		canExp = size <= 50000;//限制5w条不允许导出
    ...
    function cashflowInitTable(corpId,ModalId) {
    	$('#'+ModalId+' #cashflowList').bootstrapTable('destroy');
    	$('#'+ModalId+' #cashflowList').bootstrapTable({
    		method : "post",
    		url : "../../cashflow/list",
    		striped : false, // 表格显示条纹
    		pagination : true, // 启动分页
...
//    				 canExp = size <= 50000;//限制5w条不允许导出
    ...
    function bankRecInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #bankRecList').bootstrapTable('destroy');  
    	$('#'+ModalId+' #bankRecList').bootstrapTable({  
             method: "post", 
             url: "../../bankRec/list", 
             striped: true,  //表格显示条纹  
             pagination: true, //启动分页  
    ...
    function taxReturnsInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #taxReturnsList').bootstrapTable('destroy');
    	$('#'+ModalId+' #taxReturnsList').bootstrapTable({  
             method: "post", 
             url: "../../taxReturns/list", 
             striped: true,  //表格显示条纹  
             pagination: true, //启动分页  
    ...
    //初始化信用及诉讼信息表------------------------------------------------------------
    function initCreditLtgInfoTable(corpId,ModalId){
    	corpInitTable(corpId,ModalId);//企业征信
    	shareInitTable(corpId,ModalId);//个人征信
    	orgnInitTable(corpId,ModalId);//机构借款统计表
    	guaranteeInitTable(corpId,ModalId);//对外担保明细
    	litigantInitTable(corpId,ModalId);//诉讼情况表
    }

    function corpInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #corpListTable').bootstrapTable('destroy');
    	$('#'+ModalId+' #corpListTable').bootstrapTable({
    		method : "post",
    ...
    function shareInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #shareListTable').bootstrapTable('destroy');  
    	$('#'+ModalId+' #shareListTable').bootstrapTable({  
    		method : "post",
    		url : "../../creditReport/list",
    		striped : true, // 表格显示条纹
    		pagination : true, // 启动分页
    ...
    function orgnInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #orgnListTable').bootstrapTable('destroy');
    	$('#'+ModalId+' #orgnListTable').bootstrapTable({  
             method: "post", 
    ...
    function guaranteeInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #guaranteeListTable').bootstrapTable('destroy');  
    	$('#'+ModalId+' #guaranteeListTable').bootstrapTable({  
             method: "post", 
    ...
    function litigantInitTable(corpId,ModalId) { 
    	$('#'+ModalId+' #litigantListTable').bootstrapTable('destroy');
    	$('#'+ModalId+' #litigantListTable').bootstrapTable({  
             method: "post", 
    ...
    //初始化业务信息中的表--------------------------------------------------------
    function initBusiInfoTable(corpId,ModalId){
    	initTableBid(corpId,ModalId);//初始化招/投标合同表
    	initTableSale(corpId,ModalId);//初始化贸易/销售合同表
    }

    function initTableBid(corpId,ModalId) { 
    	$('#'+ModalId+' #userListBidTablel').bootstrapTable('destroy');
    	$('#'+ModalId+' #userListBidTablel').bootstrapTable({  
             method: "post", 
    ...
    function initTableSale(corpId,ModalId) { 
    	$('#'+ModalId+' #userListSaleTablel').bootstrapTable('destroy');
    	$('#'+ModalId+' #userListSaleTablel').bootstrapTable({  
             method: "post", 
             url: "../../corpContract/list", 
             striped: true,  //表格显示条纹  
             pagination: true, //启动分页  
    ...
    //放款审批中的表格------------------------------------------------------------------------
    function initLoanApprovalTable(corpId,ModalId){
    	initReceiveAccountTable(corpId);
    }
    function initReceiveAccountTable(corpId,ModalId){
    	$('#'+ModalId+' #receiveAccountManagerList').bootstrapTable('destroy');  
    	$('#'+ModalId+' #receiveAccountManagerList').bootstrapTable({  
    		method: "post", 
    ...
    //附件信息的----------------------------------------------------------------------
    function initAttachmentDetails(corpId,ModalId){
    	initAttachment('base',corpId,ModalId);
    	initAttachment('finance',corpId,ModalId);
    	initAttachment('credit',corpId,ModalId);
    	initAttachment('contract',corpId,ModalId);
    }

    var moduleType;
    let initAttachment = function(type,corpId,ModalId) {
    	$('#'+ModalId+' #' + type + 'Attachment').bootstrapTable('destroy');
    	$('#'+ModalId+' #' + type + 'Attachment').bootstrapTable({
    		method : "post",
    		url : "../../uploadFile/list",
    		striped : false, //表格显示条纹  
    		pagination : true, //启动分页  
    ...
    FlowMngCommon.setSupplierBaseInfo = function(corpId){
    	...
    	var options = {
    			...
    			callBackFun : function(data) {
    				if(data.result==0){
    					 CloudUtils.setForm(data.dataList[0],"projectSetupModal #splinfoForm");
    					 CloudUtils.setForm(data.dataList[0],"projectTuningModal #splinfoForm");
    					return true;
    				}else{
    					bootbox.alert(data.resultNote);
    					return false;
