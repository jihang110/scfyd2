...
import com.ut.scf.reqbean.sys.MenuUpdateReqBean;
...
public class MenuController extends BaseJsonController {
	...
	public @ResponseBody BaseRespBean addMenu(@Valid @RequestBody MenuAddReqBean reqBean,
		...
		respBean = this.menuService.addMenu(paramMap);
		return respBean;
	}
	...
	public @ResponseBody BaseRespBean updateMenu(@Valid @RequestBody MenuUpdateReqBean reqBean,
			BindingResult bindingResult) {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
		...
		Map<String, Object> paramMap = BeanUtil.beanToMap(reqBean);
		respBean = this.menuService.updateMenu(paramMap);
		return respBean;
	}
