package com.ut.scf.respbean.bpm;

import com.ut.scf.respbean.BaseRespBean;

public class FKAddRespBean extends BaseRespBean {
	
	/** 应收账款总额 */
	private String arAmt;
	/** 币种 */
	private String ccy;
	/** 供应商编号id */
	private String custNo;
	/** 起始日 */
	private String valDt;
	/** 组织机构代码 */
	private String orgNo;
	/** 供应商编号id */
	private String coreCorpNo;
	/** 合同金额 */
	private String creditLimit;
	/** 核心企业名称 */
	private String coreCorpName;
	/** 放款金额 */
	private String loanAmt;
	/** 融资比例 */
	private String loanRt;
	/** 项目编号 */
	private String proNo;
	/** 项目名称*/
	private String proName;
	/** 供应商名称 */
	private String custName;
	/** 合同反馈序号 */
	private String cntrctSerialNo;
	/** 到期日 */
	private String dueDt;
	/** 合同编号 */
	private String cntrctNo;
	/**
	 * 立项发起日期
	 */
	private String proMakeDate;
	public String getArAmt() {
		return arAmt;
	}
	public void setArAmt(String arAmt) {
		this.arAmt = arAmt;
	}
	public String getCcy() {
		return ccy;
	}
	public void setCcy(String ccy) {
		this.ccy = ccy;
	}
	public String getCustNo() {
		return custNo;
	}
	public void setCustNo(String custNo) {
		this.custNo = custNo;
	}
	public String getValDt() {
		return valDt;
	}
	public void setValDt(String valDt) {
		this.valDt = valDt;
	}
	public String getOrgNo() {
		return orgNo;
	}
	public void setOrgNo(String orgNo) {
		this.orgNo = orgNo;
	}
	public String getCoreCorpNo() {
		return coreCorpNo;
	}
	public void setCoreCorpNo(String coreCorpNo) {
		this.coreCorpNo = coreCorpNo;
	}
	public String getCreditLimit() {
		return creditLimit;
	}
	public void setCreditLimit(String creditLimit) {
		this.creditLimit = creditLimit;
	}
	public String getCoreCorpName() {
		return coreCorpName;
	}
	public void setCoreCorpName(String coreCorpName) {
		this.coreCorpName = coreCorpName;
	}
	public String getLoanAmt() {
		return loanAmt;
	}
	public void setLoanAmt(String loanAmt) {
		this.loanAmt = loanAmt;
	}
	public String getLoanRt() {
		return loanRt;
	}
	public void setLoanRt(String loanRt) {
		this.loanRt = loanRt;
	}
	public String getProNo() {
		return proNo;
	}
	public void setProNo(String proNo) {
		this.proNo = proNo;
	}
	public String getCustName() {
		return custName;
	}
	public void setCustName(String custName) {
		this.custName = custName;
	}
	public String getCntrctSerialNo() {
		return cntrctSerialNo;
	}
	public void setCntrctSerialNo(String cntrctSerialNo) {
		this.cntrctSerialNo = cntrctSerialNo;
	}
	public String getDueDt() {
		return dueDt;
	}
	public void setDueDt(String dueDt) {
		this.dueDt = dueDt;
	}
	public String getCntrctNo() {
		return cntrctNo;
	}
	public void setCntrctNo(String cntrctNo) {
		this.cntrctNo = cntrctNo;
	}
	public String getProMakeDate() {
		return proMakeDate;
	}
	public void setProMakeDate(String proMakeDate) {
		this.proMakeDate = proMakeDate;
	}
	public String getProName() {
		return proName;
	}
	public void setProName(String proName) {
		this.proName = proName;
	}
	
}
