...
public class RoleController extends BaseJsonController {
	...
	@RequestMapping(value = "/role/listByCorp", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean roleListByCorp(HttpSession httpSession,
			@RequestBody RoleListReqBean reqBean, BindingResult bindingResult) {
		BaseRespBean respBean = new BaseRespBean();
		...
		respBean = this.roleService.getRoleListByCorpId(reqBean.getCorpId());
		return respBean;
	}
