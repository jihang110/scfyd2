...
var modFun = function(row, type, num) {
    ...
    if (row.operYear!=null && row.operYear!='') {
    	$(".operYear").val(CloudUtils.dateFormat(row.operYear, 'yyyy'));
    }
    if(row.startDate!=null&&row.startDate!=''){
...
var financeNumFormat = function(){
	...
	$("#debitAmount").number(true, 2).not('[readonly]').attr('value','0.00');
	$("#creditAmount").number(true, 2).not('[readonly]').attr('value','0.00');
	$("#purchasesTax").number(true, 2).not('[readonly]').attr('value','0.00');
	$("#salesTax").number(true, 2).not('[readonly]').attr('value','0.00');
}