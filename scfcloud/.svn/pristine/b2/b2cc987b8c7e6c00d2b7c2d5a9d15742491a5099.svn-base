...
//		var ss= data2string.replace(/[\s\S]*"menuId"/g,'JH');
	...
	// 日期格式化
	CloudUtils.dateFormat = function(time, format) {
		var t = new Date(time);
	    var tf = function(i) {
	    	return (i < 10 ? '0' : '') + i
	    };
	...
	// 数据是否有效， 否为0
	CloudUtils.transform = function(value) {
	 ...
	 * hiddenId 隐藏id
	 */
	CloudUtils.searchMoreIsHidden = function(searchMoreId,hiddenId) {
		$("#"+searchMoreId).click(function(){
			var i = $("#"+searchMoreId).children('i');
	 ...
	 * 
	 */
	CloudUtils.setMainHeight = function() {
		var mainFrame = parent.document.getElementById('mainFrame')||null;
		mainFrame.height= $(document.body).height()+20;
	}
